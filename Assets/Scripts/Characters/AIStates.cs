...
public sealed class AIFindAmmo : AIState
{
	...
	public override void Retarget()
	{
		...
		foreach (GameObject obj in items)
		{
			...
			System.Collections.Generic.List<Vector2> path = GameController.Instance.Pathfind(m_ai.transform.position, tf.position, Vector2.zero, m_ai.GetComponent<Collider2D>().bounds.extents.y, !m_ai.HasFlying && m_ai.jumpTakeOffSpeed <= 0.0f ? 0.0f : float.MaxValue); // TODO: limit to max jump height once pathfinding takes platforms into account?
			if (path == null)
			{
