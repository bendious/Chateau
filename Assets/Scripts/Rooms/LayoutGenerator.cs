...
public class LayoutGenerator
{
	...
	private static readonly ReplacementRule[] m_rules =
	{
		...
		new(Node.Type.Zone3, new() { new(Node.Type.Entrance, new() { new(Node.Type.SequenceSmall, new() { new(Node.Type.AreaDivider, new() { new(Node.Type.SequenceMedium, new() { new(Node.Type.Lock, new() { new(Node.Type.AreaDivider, new() { new(Node.Type.SequenceExtraLarge, new() { new(Node.Type.SecretHintGroup), new(Node.Type.Lock, new() { new(Node.Type.AreaDivider, new() { new(Node.Type.Boss, new() { new(Node.Type.TightCoupling, new() { new(Node.Type.Npc), new(Node.Type.ExitDoor), new(Node.Type.FinalHint) }) }) }) }) }) }) }) }) }) }) }) }),
