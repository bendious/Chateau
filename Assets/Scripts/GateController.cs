...
public class GateController : MonoBehaviour, IUnlockable
{
	...
	public void SpawnKeys(RoomController lockRoom, RoomController[] keyRooms)
	{
		...
		Vector3 spawnPos = /*lock*/keyRooms[Random.Range(0, keyRooms.Length)].ChildPosition(false, null, true, false) + new Vector3(0.0f, -lockPrefab.GetComponentsInChildren<SpriteRenderer>().Min(renderer => renderer.bounds.min.y), 1.0f); // TODO: put lock in lockRoom once gates are placed correctly
